...
class ModelSpeech():  # 语音模型类
    ...
    def CreateModel(self):
        ...
        layer_h18 = Dense(self.MS_OUTPUT_SIZE, use_bias=True, kernel_initializer='he_normal')(layer_h17)  # 全连接层 输出层
    ...
    #
    #     # 主模型
    #     input_data = Input(name='the_input', shape=(self.AUDIO_LENGTH, self.AUDIO_FEATURE_LENGTH, 1))  # 输入
    #     layer_h1 = Conv2D(32, (3, 3), use_bias=False, activation='relu', padding='same',
    #                       kernel_initializer='he_normal')(input_data)  # 卷积层
    #     layer_h1 = Dropout(0.05)(layer_h1)
    #     layer_h2 = Conv2D(32, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(
    ...
    #
    #     # 冻结参数
    #     loss_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='the_input|dense_1|dense_2')
    #
    #     opt = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=10e-8).minimize(loss_out,
    #                                                                                                       var_list=loss_vars)
    #     # 加载参数
    #     ms = ModelSpeech(datapath)
    #     ms.LoadModel('model_speech/speech_model251_e_0_step_625000.model')
    #
    #     # 冻结参数
    #     # ms.trainable = False
    #
    ...
    #     # ms.TrainModel('dataset', epoch=50, batch_size=16, save_step=500)
    #
    #     # ****** 权重探针 ********
    #     # a = model.get_weights()
    #     # print('权重:', a[1])
    ...
    def TrainModel(self, datapath, epoch=2, save_step=1000, batch_size=32,
        ...
        for layer in self._model.layers:
            ...
            if layerName.startswith("conv2d_3") or layerName.startswith("conv2d_4") or layerName.startswith(
                    "conv2d_5") or layerName.startswith("conv2d_6") or layerName.startswith(
                "conv2d_7") or layerName.startswith("conv2d_8")or layerName.startswith("conv2d_9"):
...
if (__name__ == '__main__'):
    ...
    system_type = plat.system()  # 由于不同的系统的文件路径表示不一样，需要进行判断
    if (system_type == 'Windows'):
        datapath = 'F:\\ASRT_SpeechRecognition'
        modelpath = modelpath + '\\'
    elif (system_type == 'Linux'):
